      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
         {teams.map((team, index) => (
            <div key={index} className="flex flex-col gap-5 p-6 shadow-lg">
               <img src={team.img} alt={team.name} width={493} height={476} />

               <div className="flex flex-col gap-5">
                  <h5 className="text-[34px] font-bold">{team.name}</h5>

                  <p className="font-normal text-2xl text-gray-600">{team.job}</p>

                  <div className="flex gap-10">
                     <Link className="text-xl font-semibold text-gray-500 flex gap-2 items-center" to={"/"}>
                        LINKEDIN
